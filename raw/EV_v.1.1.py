...
def build_model(data):
    ...
    YE = m.addVars(Np, lb=0.0, vtype=GRB.CONTINUOUS, name="YE")
    ye = m.addVars(Np, lb=0.0, vtype=GRB.CONTINUOUS, name="ye")
    ...
    for i, j in A:
        ...
        for v in V:
            for t in T:
                charge_time_i = g_e * (YE[i] - ye[i]) if i in S_set else 0.0
                m.addConstr(
                    tau[i, t]
                    + st.get(i, 0.0)
    ...
    if crew_members:
        ...
        for t in T:
            ...
            for i in Np:
                ...
                for v in V:
                    if (i, 0) in A:
                        extra_charge = g_e * (YE[i] - ye[i]) if i in S_set else 0.0
    ...
    for i, j in A:
        ...
        for v in V:
            for t in T:
                m.addConstr(ye[j] >= 0, name=f"c20_lb_{i}_{j}_{v}_{t}")
                m.addConstr(
                    ye[j] <= ye[i] - (h_e * d[i, j]) * x[i, j, v, t] + Q[v] * (1 - x[i, j, v, t]),
    ...
    for i, j in A:
        ...
        for v in V:
            for t in T:
                m.addConstr(ye[j] >= 0, name=f"c21_lb_{i}_{j}_{v}_{t}")
                m.addConstr(
                    ye[j] <= YE[i] - (h_e * d[i, j]) * x[i, j, v, t] + Q[v] * (1 - x[i, j, v, t]),
    ...
    for i in Np:
        ...
        if i in S_set:
            ...
            m.addConstr(YE[i] >= ye[i], name=f"c22_station_recharge_lb_{i}")
        else:
            m.addConstr(YE[i] == ye[i], name=f"c22_no_recharge_{i}")
        for v in V:
            m.addConstr(YE[i] <= Q[v], name=f"c22_ub_{i}_{v}")

    if S_set:
        max_q = max(float(Q[v]) for v in V) if V else 0.0
        for s in S_set:
            outflow_s = gp.quicksum(
                x[s, j, v, t]
                for j in Np
                for v in V
                for t in T
            ...
            m.addConstr(
                YE[s] - ye[s] <= max_q * outflow_s,
                name=f"c22_station_recharge_bigM_{s}",
            )
    ...
    for i in Np:
        ...
        for v in V:
            for t in T:
                if (i, 0) in A:
                    m.addConstr(
                        ye[i] >= (h_e * d.get((i, 0), 0.0)) * x[i, 0, v, t],
    ...
    for j in Np:
        ...
        for v in V:
            for t in T:
                if (0, j) in A:
                    m.addConstr(
                        ye[j] <= Q[v] - (h_e * d.get((0, j), 0.0)) * x[0, j, v, t] + 100000.0 * (1 - x[0, j, v, t]),
                        name=f"c_full_depot_ub_{j}_{v}_{t}",
                    )
                    m.addConstr(
                        ye[j] >= Q[v] - (h_e * d.get((0, j), 0.0)) - 100000.0 * (1 - x[0, j, v, t]),
...
def print_ev_solution(data, model):
    ...
    def charge_amount(node):
        if node in S_set and YE is not None and ye is not None and node in YE and node in ye:
            return max(0.0, float(YE[node].X - ye[node].X))
        return 0.0
        ...
        for idx in range(len(route)):
            ...
            if idx == len(route) - 1:
                ...
                if prev_node in S_set:
                    end_val += g_e * charge_amount(prev_node)
